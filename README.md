...
The deck is generated from 300 distinct snippet topics, with 10 cards per topic. Cards are interleaved by topic inside each module, so the deck does not begin with long runs of nearly identical `const` or `let` examples.

## Files

- `dist/javascript-code-reading-3000.tsv` - the Anki import file with 3000 Basic notes.
- `dist/javascript-code-reading-3000.apkg` - the Anki package file with the same 3000 notes.
- `dist/javascript-code-reading-3000.summary.json` - module counts and output metadata.
...
```text
dist/javascript-code-reading-3000.apkg
```
...
```text
dist/javascript-code-reading-3000.tsv
```
...
```text
JavaScript Code Reading::3000 Snippets
```
...
1. Open Anki Desktop.
2. Import `dist/javascript-code-reading-3000.apkg` with `File -> Import`.
...
7. Visit AnkiWeb and confirm the `JavaScript Code Reading::3000 Snippets` deck appears.
...
7. Open the `JavaScript Code Reading::3000 Snippets` deck.

Direct local import on Android is also possible if you copy `dist/javascript-code-reading-3000.apkg` onto the device and open it with AnkiDroid. The Desktop -> AnkiWeb -> AnkiDroid route is still the most reliable path when you already use Anki sync.
...
- Back: intent, annotated code line by line, and common use cases.
